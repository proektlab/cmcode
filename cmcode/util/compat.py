...
def _populate_missing_fields(loaded_info: dict[str, Any]):
    ...
    if 'tag' not in loaded_info:
        loaded_info['tag'] = None

    if 'plane_tifs' not in loaded_info:
        loaded_info['plane_tifs'] = None

    if 'mc_result' not in loaded_info:
...
def _validate_or_write_missing_params_files(sessdata: 'cma.SessionAnalysis'):
    ...
    if sessdata.cnmf_fit_filename is not None:
        try:
            ...
            cnmf_run_params, invalid_stage = sessdata.params.change_from_cnmf_h5_and_get_stage_to_invalidate(
                sessdata.cnmf_fit_filename, metadata=sessdata.metadata
            )
