...
class SessionAnalysis:
    ...
    def do_cnmf_evaluation(self, recalc=False):
            ...
            try:
                ...
                if self.params.do_params_match(old_eval_params, self.metadata, stage=cmp.AnalysisStage.EVAL):
                    ...
                    if est.idx_components_eval is not None and est.idx_components_bad_eval is not None:
                        ...
                        self._cnmf_fit = None  # trigger reload
                        if self.cnmf_fit is None:
        ...
        evaluate_cnmf(self.cnmf_fit, self.mmap_file_transposed, snr_type=snr_type,
...
def get_session_analysis_file_pattern(mouse_id: Union[str, int], sess_id: Union[int, str], tag: Optional[str]) -> str:
    if isinstance(sess_id, str):
        if tag is not None:
            raise ValueError('Cannot pass both a string sess_id and a tag')
        sess_name = naming.format_sess_name(sess_id, underscore=True, zero_padded=True)
